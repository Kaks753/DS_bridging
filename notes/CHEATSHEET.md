...
## 0 — Absolute Basics (Python from Zero)
- **Code** = precise instructions a computer runs top-to-bottom; a **notebook** is code split into runnable **cells** (Shift+Enter runs one).
- **Variable** = a labelled box: `x = 5`. **Types**: `int`/`float` (numbers), `str` (text), `bool` (`True`/`False`), `list` (ordered), `dict` (key→value).
- **`if`/`elif`/`else`** = decisions; **`for ... in ...`** = repeat over items; indentation (4 spaces) defines the block.
- **Function** = a reusable mini-machine: `def add(a, b): return a + b`; call it as `add(2, 3)`.
- **Import** = borrow other people's code: `import numpy as np`, then `np.array(...)`.
- **Read the error bottom-up**: the last line names the error type + message (e.g. `NameError`, `KeyError`) — it usually tells you exactly what to fix.
...
## 9 — Evaluation, CV, Pipelines, Tuning
...
## 16 — Probability, Combinatorics & Bayes
...
## 18 — OOP + Linear Algebra & Calculus
